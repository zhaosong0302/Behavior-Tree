...
///   当前处于的巡逻类型（int）
/// </summary>
public class PatrolState//这是从有限状态机AI中移植过来的代码 我们只需要对它进行一定的修改即可
{
    //所有巡逻方式都会用到的成员
    ...
    public bool Update()
    {
        ...
        switch (type)
        {
            ...
            case 1:
                ...
                //命令它播放某一个动画
                monster.ChangeAction(actionType);
                break;
            //在圆范围内随机移动的表现
            case 2:
            //在多个点上来回移动或者循环移动的表现
            case 3:
                ...
                if(waitTime <= 0)
                {
                    ...
                    if (Vector3.Distance(targetPos, monster.nowPos) < 0.2f)
                    {
                        isChangePos = true;
                        //到达目标点后 停止移动
                        monster.StopMove();
        ...
        ////脱离巡逻判断
        ////通用处理 当自己和目标处于一定距离内就进行巡逻状态脱离
        //if(Vector3.Distance(monster.nowPos, monster.targetObjPos) < checkDis)
        //{
        //    //切换到追逐状态
    ...
    {
        if (Vector3.Distance(monster.nowPos, monster.bornPos) > 15)
        {
            monster.StopMove();
            return false;
        }
        //0111
        monster.ClearState(7);
        return true;
    }
